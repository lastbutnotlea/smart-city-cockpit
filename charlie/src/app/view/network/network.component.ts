...
export class NetworkComponent implements OnInit {
  ...
  private getMapData(): void {
    this.http.getMapDataStations().subscribe(data => {
        this.testStations = data;
      },
      err => {
        console.log('Could not fetch Map-Data for stations.');
      });
    this.http.getMapDataLines().subscribe(data => {
        this.testLines = data;
      },
      err => {
        console.log('Could not fetch Map-Data for lines.');
      });
    this.http.getMapDataConnections().subscribe(data => {
        this.testConnections = data;
      },
      err => {
        console.log('Could not fetch Map-Data for connections.');
      });
    this.drawTubeMap(this.mapCreator.createMap(this.testStations, this.testLines, this.testConnections));
    this.addLineEvents();
  }
